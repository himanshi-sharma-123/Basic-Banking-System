...
router.get("/view/:id", (req, res) => {
  ...
  Promise.all([User.find({ _id: id }), User.find()])
    .then(([userResult, allUserResult]) => {
...
//- Delete History
router.get("/historydelete/:id", async (req, res) => {
  try {
    const id = req.params.id;
    ...
    if (err) {
      ...
      adata.forEach((a) => {
        if (a.name != reciverName || a.email != reciverEmail) {
          res.render("success", {
            title: "sucess",
            errmsg: "Reciver Details Not Matched!",
            value: "",
            msg: "",
          });
        } else {
          var senderData = User.find({ _id: senderId });
          var reciverData = User.find({
            name: reciverName,
            email: reciverEmail,
          });
                ...
                if (err) {
                  ...
                  sdata.forEach(async (e) => {
                    if (e.amount < sendAmount || e.name === reciverName) {
                      res.render("success", {
                        title: "sucess",
                        errmsg: "Not Proceed Due TO Incorrect Information",
                        value: "",
                        msg: "",
                      });
                    } else {
                      let senderUpdateAmount =
                        parseInt(e.amount) - parseInt(sendAmount);
                      await User.findOneAndUpdate(
                        { _id: senderId },
                        { $set: { amount: senderUpdateAmount } }
                      );
                      // -----------------
                      addHistory
                        .save()
                      ...
                      rdata.forEach(async (e) => {
                        let reciverUpdateAmount =
                          parseInt(e.amount) + parseInt(sendAmount);
                        await User.findOneAndUpdate(
                          { name: reciverName },
                          { $set: { amount: reciverUpdateAmount } }
                        );
                      });
                      res.render("success", {
                        title: "sucess",
                        msg: "Transaction Sucessfully Done!",
                        value: "true",
                      });
                    }
                  });
